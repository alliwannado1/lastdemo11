...
public class VanBan extends Asset<VanBan> {
	...
	@ManyToOne
	@JoinTable(name = "vanban_capbanhanh", joinColumns = { @JoinColumn(name = "vanban_id") }, inverseJoinColumns = {
			@JoinColumn(name = "capbanhanh_id") })
	...
	@ManyToOne
	@JoinTable(name = "vanban_linhvuc", joinColumns = { @JoinColumn(name = "vanban_id") }, inverseJoinColumns = {
			@JoinColumn(name = "linhvuc_id") })
	...
	@ManyToOne
	@JoinTable(name = "vanban_coquan", joinColumns = { @JoinColumn(name = "vanban_id") }, inverseJoinColumns = {
			@JoinColumn(name = "coquan_id") })
	...
	@ManyToOne
	@JoinTable(name = "vanban_loaivanban", joinColumns = { @JoinColumn(name = "vanban_id") }, inverseJoinColumns = {
			@JoinColumn(name = "loaivanban_id") })
	...
	@Command
	public void saveVanBan(@BindingParam("list") final Object listObject, @BindingParam("attr") final String attr,
	...
	@Command
	public void xuatBan(@BindingParam("list") final Object listObject, @BindingParam("attr") final String attr,
			@BindingParam("wdn") final Window wdn, @BindingParam("stt") final boolean stt) {
		if (stt) {
				...
				Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener<Event>() {
					...
					public void onEvent(final Event event) {
						if (Messagebox.ON_OK.equals(event.getName())) {
							file = null;
							BindUtils.postNotifyChange(null, null, VanBan.this, "file");
						}
					}
				});
	}
	...
	public AbstractValidator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(final ValidationContext ctx) {
				/*
				 * FileEntry file = getFile(); if("".equals(file.getFileUrl()))
				 * { addInvalidMessage(ctx, "uploadbtn",
				 * "Bạn chưa upload file đính kèm."); }
				 */
				if (file == null) {
					addInvalidMessage(ctx, "uploadbtn", "Bạn chưa upload file đính kèm.");
				}
				Date fromDate = getNgayBanHanh();
				Date toDate = getNgayHieuLuc();
				if (fromDate != null && toDate != null) {
					if (fromDate.compareTo(toDate) > 0) {
						addInvalidMessage(ctx, "dateErr", "Ngày hiệu lực phải lớn hơn hoặc bằng ngày ban hành.");
					}
				}
